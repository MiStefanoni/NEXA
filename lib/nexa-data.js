export const CATEGORY_ORDER = [
  "saude-bem-estar-cuidado",
  "servicos-profissionais-negocios",
  "home-and-family-care",
  "educacao-desenvolvimento-consultoria",
];

export const CATEGORY_META = {
  "saude-bem-estar-cuidado": {
    pt: {
      title: "Saúde, Bem-Estar e Cuidado",
      description:
        "Psicólogas, nutricionistas, fisioterapeutas, personal trainers, terapeutas ocupacionais, fonoaudiólogas, médicas, dentistas, terapeutas, profissionais de estética.",
      note: "Atendimento especializado com foco em saúde, acolhimento e confiança.",
      pageDescription:
        "Encontre profissionais de saúde, bem-estar e cuidado oferecendo suporte especializado, atendimento confiável e experiências mais seguras para suas clientes.",
    },
    en: {
      title: "Health, Wellness & Care",
      description:
        "Psychologists, nutritionists, physical therapists, personal trainers, occupational therapists, speech therapists, doctors, dentists, therapists, and beauty professionals.",
      note: "Specialized care centered on wellbeing, trust, and client support.",
      pageDescription:
        "Explore health, wellness, and care professionals offering trusted expertise, thoughtful support, and structured client experiences.",
    },
  },
  "servicos-profissionais-negocios": {
    pt: {
      title: "Serviços Profissionais e Negócios",
      description:
        "Advogadas, contadoras, consultoras empresariais, recrutadoras, profissionais de RH, estrategistas de marca, arquitetas, designers, programadoras, assessoras e especialistas B2B.",
      note: "Serviços estruturados para operação, crescimento e suporte profissional.",
      pageDescription:
        "Descubra profissionais que apoiam negócios, operações e entregas especializadas com clareza, organização e credibilidade.",
    },
    en: {
      title: "Professional & Business Services",
      description:
        "Lawyers, accountants, business consultants, recruiters, HR professionals, brand strategists, architects, designers, developers, assistants, and B2B specialists.",
      note: "Structured services for operations, growth, and professional support.",
      pageDescription:
        "Discover professionals supporting businesses, operations, and specialized delivery with clarity, structure, and credibility.",
    },
  },
  "home-and-family-care": {
    pt: {
      title: "Casa e Família",
      description:
        "Babás, chefs e cozinheiras, cuidadora de idosos, diaristas, personal organizers, serviços gerais e construção.",
      note: "Serviços domésticos, familiares e de manutenção com confiança e apoio prático.",
      pageDescription:
        "Explore profissionais que apoiam a rotina da casa e da família com organização, cuidado, manutenção e serviços práticos.",
    },
    en: {
      title: "Home and Family Care",
      description:
        "Nannies, chefs and cooks, elder care providers, cleaners, personal organizers, and general home and construction services.",
      note: "Trusted home, family, and maintenance services with practical day-to-day support.",
      pageDescription:
        "Explore professionals supporting home and family life through organization, care, upkeep, and practical residential services.",
    },
  },
  "educacao-desenvolvimento-consultoria": {
    pt: {
      title: "Educação, Desenvolvimento e Consultoria",
      description:
        "Professoras particulares, especialistas em idiomas, orientadoras educacionais, mentoras, consultoras de carreira, educadoras, facilitadoras.",
      note: "Aprendizado, orientação e desenvolvimento com método e clareza.",
      pageDescription:
        "Explore profissionais focadas em ensino, desenvolvimento e consultoria com abordagens estruturadas para aprendizado e crescimento.",
    },
    en: {
      title: "Education, Development & Consulting",
      description:
        "Private tutors, language specialists, educational advisors, mentors, career consultants, educators, and facilitators.",
      note: "Learning, guidance, and development delivered with structure and clarity.",
      pageDescription:
        "Explore professionals focused on education, development, and consulting through structured guidance for learning and growth.",
    },
  },
};

export const UI = {
  pt: {
    htmlLang: "pt-BR",
    rootPath: "/pt/index.html",
    homePath: "/pt",
    aboutPath: "/pt/about",
    categoriesPath: "/pt/categories",
    professionalsPath: "/pt/professionals",
    applyPath: "/pt/apply",
    guidelinesPath: "/pt/guidelines",
    privacyPath: "/pt/privacy-policy",
    termsPath: "/pt/terms-of-use",
    nav: {
      about: "Sobre a Nexa",
      categories: "Explorar categorias",
      professionals: "Explorar profissionais",
      howItWorks: "Como funciona",
      applyNow: "Entre para a Nexa",
      menu: "Menu",
    },
    home: {
      title: "Nexa | Diretório Profissional Multilíngue para Mulheres",
      description:
        "Nexa é um diretório profissional que conecta mulheres profissionais a clientes por meio de visibilidade confiável, perfis bem apresentados e contatos estruturados.",
      eyebrow: "Visibilidade profissional, pensada com cuidado",
      heading: "Mulheres profissionais, novas conexões, mais oportunidades",
      body:
        "A Nexa é uma plataforma criada para ampliar a presença de mulheres no mercado, conectando talentos a clientes, empresas e parceiros que procuram serviços com confiança, qualidade e profissionalismo. Aqui, cada perfil ganha uma vitrine própria para mostrar experiências, serviços e diferenciais abrindo caminhos para novas conexões e oportunidades reais.",
      primaryCta: "Encontrar profissionais",
      secondaryCta: "Entrar para a Nexa",
      heroAlt: "Mulher usando a plataforma Nexa",
      howEyebrow: "Como funciona",
      howTitle: "Do cadastro ao contato: uma jornada simples, segura e profissional",
      howSteps: [
        {
          label: "Passo 1",
          title: "Candidate-se para fazer parte",
          body: "Envie suas informações para análise. A curadoria da Nexa ajuda a manter a plataforma organizada, confiável e alinhada ao seu propósito: valorizar mulheres profissionais e seus talentos.",
        },
        {
          label: "Passo 2",
          title: "Crie seu perfil profissional",
          body: "Apresente sua trajetória, seus serviços, sua experiência e seus diferenciais em uma página clara, estruturada e fácil de conhecer.",
        },
        {
          label: "Passo 3",
          title: "Receba conexões qualificadas",
          body: "Clientes, empresas e parceiros podem encontrar seu perfil por área de atuação, localização e formato de atendimento, facilitando contatos mais diretos e relevantes.",
        },
      ],
      categoriesEyebrow: "Categorias",
      categoriesTitle:
        "Encontre mulheres profissionais em áreas essenciais para a sua vida, sua rotina e seu negócio",
      featuredEyebrow: "Profissionais em destaque",
      featuredTitle: "Conheça as profissionais que já fazem parte da Nexa",
      joinEyebrow: "ENTRE PARA A NEXA",
      joinTitle: "Faça parte de uma plataforma criada para valorizar o seu trabalho",
      joinBody:
        "A Nexa reúne mulheres que querem apresentar seus serviços com mais profissionalismo, fortalecer sua presença no mercado e ser encontradas por quem procura talento, confiança e qualidade. Ao entrar para a Nexa, seu trabalho ganha um espaço próprio para mostrar quem você é, o que você faz e por que a sua experiência merece ser reconhecida. Um perfil pensado para valorizar sua trajetória Apresente seus serviços, sua experiência, seus diferenciais e sua forma de atuação em uma página clara, profissional e fácil de compartilhar. Mais chances de ser encontrada por quem procura o que você oferece A Nexa organiza profissionais por categoria, localização e formato de atendimento, facilitando o encontro entre mulheres qualificadas e pessoas interessadas em contratar. Uma presença profissional mais forte e organizada Seu trabalho passa a ocupar um espaço confiável, com foco em visibilidade, conexão e novas oportunidades sem depender apenas das redes sociais.",
      applyTitle: "Envie sua candidatura",
      applyBody:
        "Preencha suas informações para que a equipe da Nexa conheça melhor sua atuação profissional e avalie seu perfil para fazer parte da plataforma.",
    },
    categoriesPage: {
      title: "Categorias | Nexa",
      description: "Explore categorias de serviço no diretório multilíngue da Nexa.",
      eyebrow: "Explorar categorias",
      heading: "Conheça os serviços liderados por mulheres disponíveis na Nexa",
      body: "Navegue por categorias curadas para facilitar descoberta, comparação e contatos mais profissionais.",
      cta: "Ver profissionais",
    },
    professionalsPage: {
      title: "Profissionais | Nexa",
      description: "Explore todas as profissionais listadas na Nexa em português.",
      eyebrow: "Profissionais",
      heading: "Explore o diretório completo da Nexa",
      body: "Conheça mulheres profissionais em todas as categorias ativas da Nexa. O diretório é alimentado por uma base compartilhada para manter os perfis consistentes conforme o catálogo cresce.",
      searchPlaceholder: "Buscar uma profissional",
    },
    applyPage: {
      title: "Candidatar-se | Nexa",
      description: "Candidate-se para participar da Nexa como profissional listada.",
      eyebrow: "Aplicação profissional",
      heading: "Candidate-se para entrar na Nexa",
      body: "A Nexa recebe mulheres profissionais que desejam um espaço confiável e bem apresentado para divulgar seus serviços. Candidate-se para ser considerada no diretório e em uma página de perfil estruturada.",
      bullets: [
        "Análise cuidadosa para manter o diretório coerente, confiável e alinhado.",
        "Estruturas de perfil pensadas para clareza, confiança e descoberta.",
        "Caminhos de contato estruturados que reduzem ruído e melhoram alinhamento.",
      ],
      formTitle: "Formulário de aplicação profissional",
    },
    aboutPage: {
      title: "Sobre a Nexa | Nexa",
      description: "Leia o Manifesto Nexa e conheça a visão por trás da plataforma.",
      eyebrow: "Sobre a Nexa",
      heading: "Manifesto Nexa",
      intro:
        "Nossa visão",
      manifestoEyebrow: "Nossa visão",
      manifestoTitle: "Porque talento não deveria se perder no ruído",
      manifestoParagraphs: [
        "Acreditamos que profissionais incríveis não deveriam depender de algoritmo para serem encontradas.",
        "Hoje, grande parte do trabalho feminino ainda acontece em ambientes digitais informais, saturados e instáveis — onde visibilidade depende de engajamento constante, exposição pessoal e dinâmicas que pouco têm relação com competência profissional.",
        "A Nexa nasce como uma resposta a isso.",
        "Criamos uma plataforma onde mulheres possam apresentar seu trabalho de forma clara, profissional e estruturada. Um espaço onde serviços sejam descobertos pela qualidade do que oferecem — não pela capacidade de disputar atenção em redes sociais.",
        "Não acreditamos que profissionalismo precise significar rigidez.",
        "Nem que visibilidade precise custar privacidade.",
        "Nem que crescer profissionalmente deva depender de exposição constante.",
        "Acreditamos em relações mais claras entre profissionais e clientes.",
        "Em ambientes digitais mais organizados, respeitosos e sustentáveis.",
        "Em estruturas que valorizem competência, confiança e posicionamento.",
        "Mais do que uma plataforma, a Nexa é uma infraestrutura de visibilidade.",
        "Um lugar pensado para transformar presença profissional em oportunidade real.",
        "Começamos no Brasil, mas com uma visão global: construir um ambiente onde mulheres profissionais possam ser descobertas, reconhecidas e valorizadas com mais autonomia, segurança e clareza.",
        "Porque talento não deveria se perder no ruído.",
      ],
      ctaEyebrow: "Próximo passo",
      ctaTitle: "Explore a plataforma ou envie sua candidatura",
      ctaBody:
        "Se você quer conhecer as profissionais que já fazem parte da Nexa, explore o diretório. Se deseja apresentar seu trabalho na plataforma, envie sua candidatura para análise.",
      primaryCta: "Explorar profissionais",
      secondaryCta: "Entrar para a Nexa",
    },
    footer: {
      about: "Sobre a Nexa",
      howItWorks: "Como funciona",
      categories: "Explorar categorias",
      professionals: "Explorar profissionais",
      applyNow: "Entre para a Nexa",
      guidelines: "Diretrizes",
      privacy: "Privacidade",
      terms: "Política de uso",
    },
    cardCategoryCta: "Ver profissionais",
    cardProfileCta: "Ver perfil",
    directoryEmpty: "Nenhuma profissional corresponde a este filtro no momento.",
    featuredEmpty: "Nenhuma profissional em destaque corresponde a este filtro no momento.",
    verified: "Verificada",
    remote: "Remoto",
    local: "Local",
    remoteAndLocal: "Remoto e local",
    all: "Todas",
    availability: "Disponibilidade",
    profileFallback: "Este perfil está disponível atualmente em português.",
    profileNotFoundTitle: "Perfil não encontrado",
    profileNotFoundText: "Não foi possível localizar esta profissional.",
    backToProfessionals: "Voltar para profissionais",
    browseAllProfessionals: "Explorar todas as profissionais",
    backToHome: "Voltar para início",
    sections: {
      about: "Sobre",
      services: "Serviços",
      experience: "Experiência",
      portfolio: "Portfólio",
      contact: "Contato",
    },
    labels: {
      location: "Localização",
      languages: "Idiomas",
      profileType: "Tipo de perfil",
      delivery: "Formato",
      engagement: "Modelo",
      experience: "Experiência",
      clientFocus: "Foco de clientes",
      projectsDelivered: "Projetos entregues",
      email: "Email",
      website: "Website",
      social: "Rede social",
      safetyNote: "Nota de segurança",
      profileDetail: "Perfil",
      category: "Categoria",
    },
    contactViaNexa: "Entrar em contato pela Nexa",
    contactModal: {
      title: "Contate através da Nexa",
      close: "Fechar",
      name: "Nome",
      namePlaceholder: "Seu nome",
      email: "Email",
      emailPlaceholder: "voce@exemplo.com",
      phone: "Fone",
      phonePlaceholder: "(00) 00000-0000",
      message: "Mensagem",
      messagePlaceholder: "Escreva sua mensagem",
      send: "Enviar",
      validation: "Preencha Nome e Email para enviar a mensagem.",
      success: "Mensagem enviada com sucesso",
      error: "Não foi possível enviar a mensagem agora. Tente novamente.",
    },
    applicationForm: {
      validation: "Preencha os campos obrigatórios antes de enviar sua candidatura.",
      success:
        "Cadastro enviado com sucesso! Seu perfil será revisado antes de ser publicado.",
      error: "Não foi possível enviar sua candidatura agora. Tente novamente.",
      sending: "Enviando...",
    },
    legal: {
      guidelines: {
        title: "Diretrizes da Nexa | Nexa",
        description: "Diretrizes da plataforma Nexa para profissionais participantes.",
        heading: "Diretrizes da Nexa",
        intro: "Diretrizes da plataforma Nexa para profissionais participantes.",
        sections: [
          {
            title: "Diretrizes da Plataforma Nexa",
            paragraphs: [
              "A Nexa foi criada para ser um ambiente profissional estruturado, seguro e confiável, com o objetivo de aumentar a visibilidade de mulheres que oferecem serviços.",
              "Para manter a qualidade e a proposta da plataforma, todas as profissionais devem seguir as diretrizes abaixo:",
            ],
          },
          {
            title: "1. Profissionalismo",
            paragraphs: [
              "Os perfis devem refletir uma atuação profissional clara, com descrição objetiva dos serviços oferecidos.",
              "Não é permitido:",
            ],
            items: [
              "Conteúdo ofensivo, discriminatório ou inadequado",
              "Informações falsas ou enganosas",
              "Linguagem excessivamente informal ou incompatível com ambiente profissional",
            ],
          },
          {
            title: "2. Clareza de serviços",
            paragraphs: ["Cada profissional deve:"],
            items: [
              "Descrever claramente seus serviços",
              "Indicar forma de atuação (remoto/local)",
              "Informar, sempre que possível, escopo e tipo de contratação",
            ],
          },
          {
            title: "3. Uso adequado da plataforma",
            paragraphs: [
              "A Nexa é um diretório profissional, não uma rede social.",
              "Não é permitido:",
            ],
            items: [
              "Uso da plataforma para fins não relacionados à prestação de serviços",
              "Divulgação de conteúdo pessoal sem relação profissional",
              "Uso da plataforma para spam ou autopromoção fora do escopo do perfil",
            ],
          },
          {
            title: "4. Atualização de informações",
            paragraphs: ["A profissional é responsável por:"],
            items: [
              "Manter suas informações atualizadas",
              "Garantir que os dados apresentados sejam verdadeiros",
            ],
          },
          {
            title: "5. Conduta",
            paragraphs: ["Esperamos que todas as profissionais:"],
            items: [
              "Manter postura ética",
              "Respeitar clientes e outras profissionais",
              "Utilizar a plataforma de forma responsável",
            ],
          },
        ],
      },
      privacy: {
        title: "Política de Privacidade | Nexa",
        description: "Política de Privacidade da Nexa.",
        heading: "Política de Privacidade",
        intro: "Política de Privacidade da Nexa.",
        sections: [
          {
            title: "Política de Privacidade da Nexa",
            paragraphs: ["A Nexa valoriza a privacidade e a segurança das informações de suas usuárias."],
          },
          {
            title: "1. Coleta de dados",
            paragraphs: ["A Nexa coleta apenas as informações necessárias para:"],
            items: [
              "Criação e exibição do perfil profissional",
              "Funcionamento da plataforma",
              "Comunicação com a usuária",
            ],
          },
          {
            title: "2. Uso das informações",
            paragraphs: ["Os dados fornecidos serão utilizados para:"],
            items: [
              "Exibir o perfil profissional na plataforma",
              "Permitir que clientes entrem em contato",
              "Melhorar a experiência da plataforma",
            ],
          },
          {
            title: "3. Compartilhamento de dados",
            paragraphs: [
              "A Nexa não vende, não aluga e não compartilha dados pessoais com terceiros para fins comerciais.",
              "Os dados poderão ser utilizados apenas:",
            ],
            items: ["Para funcionamento interno da plataforma", "Quando exigido por obrigação legal"],
          },
          {
            title: "4. Dados públicos vs privados",
            items: [
              "Informações inseridas no perfil profissional são públicas",
              "Informações pessoais sensíveis não são solicitadas nem exibidas",
            ],
          },
          {
            title: "5. Segurança",
            paragraphs: [
              "A Nexa adota medidas razoáveis para proteger os dados das usuárias, mas não pode garantir segurança absoluta contra acessos indevidos.",
            ],
          },
          {
            title: "6. Controle dos dados",
            paragraphs: ["A usuária pode:"],
            items: [
              "Solicitar atualização ou remoção de seus dados",
              "Encerrar sua participação na plataforma a qualquer momento",
            ],
          },
        ],
      },
      terms: {
        title: "Política de Uso | Nexa",
        description: "Política de uso da plataforma Nexa.",
        heading: "Política de Uso",
        intro: "Política de uso da plataforma Nexa.",
        sections: [
          {
            title: "Política de Uso da Nexa",
            paragraphs: [
              "A Nexa se reserva o direito de revisar, aprovar, editar ou remover perfis a qualquer momento, com o objetivo de manter a qualidade e a segurança da plataforma.",
            ],
          },
          {
            title: "1. Aprovação de perfis",
            items: [
              "Todos os perfis passam por análise antes de serem publicados",
              "A Nexa pode recusar perfis que não estejam alinhados com suas diretrizes",
            ],
          },
          {
            title: "2. Remoção ou suspensão",
            paragraphs: ["A Nexa poderá remover ou suspender perfis em casos como:"],
            items: [
              "Descumprimento das diretrizes",
              "Informações falsas ou inconsistentes",
              "Uso indevido da plataforma",
              "Conduta inadequada",
            ],
          },
          {
            title: "3. Limitação de responsabilidade",
            paragraphs: ["A Nexa atua como intermediadora de visibilidade e conexão."],
            items: [
              "Não participa das negociações entre profissionais e clientes",
              "Não garante resultados, contratações ou pagamentos",
              "Não se responsabiliza por acordos realizados fora da plataforma",
            ],
          },
          {
            title: "4. Contato entre usuários",
            items: [
              "O contato entre clientes e profissionais é de responsabilidade das partes",
              "A Nexa não realiza mediação de conflitos",
            ],
          },
        ],
      },
    },
  },
  en: {
    htmlLang: "en",
    rootPath: "/en/index.html",
    homePath: "/en",
    aboutPath: "/en/about",
    categoriesPath: "/en/categories",
    professionalsPath: "/en/professionals",
    applyPath: "/en/apply",
    guidelinesPath: "/en/guidelines",
    privacyPath: "/en/privacy-policy",
    termsPath: "/en/terms-of-use",
    nav: {
      about: "About Nexa",
      categories: "Browse categories",
      professionals: "Browse professionals",
      howItWorks: "How it works",
      applyNow: "Apply now",
      menu: "Menu",
    },
    home: {
      title: "Nexa | Multilingual Professional Directory for Women",
      description:
        "Nexa is a professional directory connecting women professionals to clients through trusted visibility, polished profiles, and structured inquiries.",
      eyebrow: "Professional visibility, thoughtfully designed",
      heading: "Connecting women professionals to opportunity",
      body:
        "Nexa is a curated professional directory that helps women showcase their work, share expertise, and receive structured client inquiries in a safer, more professional environment.",
      primaryCta: "Browse professionals",
      secondaryCta: "Join as a professional",
      heroAlt: "Woman using the Nexa platform",
      howEyebrow: "How it works",
      howTitle: "A directory experience built around clarity and trust",
      howSteps: [
        {
          label: "Step 1",
          title: "Apply to join",
          body: "Submit your details for review so the directory remains professional, credible, and aligned with Nexa’s standards.",
        },
        {
          label: "Step 2",
          title: "Build your profile",
          body: "Present your services, background, and portfolio with a polished profile designed for discovery rather than social posting.",
        },
        {
          label: "Step 3",
          title: "Receive structured inquiries",
          body: "Clients reach out through guided contact flows, helping you assess fit without open direct messaging.",
        },
      ],
      categoriesEyebrow: "Categories",
      categoriesTitle: "Explore professionals across trusted service categories",
      featuredEyebrow: "Featured professionals",
      featuredTitle: "Discover standout profiles on Nexa",
      joinEyebrow: "Join Nexa",
      joinTitle: "Build a professional presence clients can trust",
      joinBody:
        "Nexa is designed for women professionals who want visibility without the noise of a social platform or the pressure of a transactional marketplace.",
      joinBullets: [
        "Curated directory placement for stronger credibility and fit.",
        "Structured profile layouts that highlight expertise and services clearly.",
        "Safer contact pathways without open direct messaging.",
      ],
      applyTitle: "Professional application",
      applyBody:
        "Fill in your information so the Nexa team can better understand your professional work and evaluate your profile to join the platform.",
    },
    categoriesPage: {
      title: "Categories | Nexa",
      description: "Explore service categories in Nexa’s multilingual directory.",
      eyebrow: "Browse categories",
      heading: "Explore women-led services available through Nexa",
      body: "Navigate curated categories that make discovery, comparison, and structured outreach easier.",
      cta: "Browse category",
    },
    professionalsPage: {
      title: "Professionals | Nexa",
      description: "Explore all professionals listed on Nexa in English.",
      eyebrow: "Professionals",
      heading: "Browse the full Nexa directory",
      body: "Discover women professionals across every active Nexa category. The directory is powered by one shared dataset so profiles stay consistent as the catalog grows.",
      searchPlaceholder: "Search professionals",
    },
    applyPage: {
      title: "Apply | Nexa",
      description: "Apply to join Nexa as a listed professional.",
      eyebrow: "Professional application",
      heading: "Apply to join Nexa",
      body: "Nexa welcomes women professionals who want a polished, trustworthy place to present their work. Apply to be considered for the directory and a structured profile page.",
      bullets: [
        "Careful review to keep the directory coherent, credible, and aligned.",
        "Profile structures designed for clarity, trust, and discoverability.",
        "Structured contact paths that reduce noise and improve fit.",
      ],
      formTitle: "Professional application form",
    },
    aboutPage: {
      title: "About Nexa | Nexa",
      description: "Learn about Nexa’s mission and how the platform helps women professionals gain stronger visibility.",
      eyebrow: "About Nexa",
      heading: "A platform built to give women professionals stronger visibility",
      intro:
        "Nexa exists to make discovery, presentation, and outreach more structured and trustworthy. Instead of relying only on fragmented referrals or social feeds, women professionals gain a clearer space to present their work with context and credibility.",
      principles: [
        {
          label: "Principle 1",
          title: "Visibility with context",
          body: "Each profile is designed to present services, experience, and strengths more clearly so clients can understand the professional behind the work.",
        },
        {
          label: "Principle 2",
          title: "More qualified connections",
          body: "The platform organizes professionals by category, location, and delivery mode to make discovery more precise and outreach more relevant.",
        },
        {
          label: "Principle 3",
          title: "A stronger professional presence",
          body: "Nexa is designed to highlight professional trajectories, build credibility, and create room for better-fit opportunities.",
        },
      ],
      storyEyebrow: "Our approach",
      storyTitle: "A structure built around trust, discovery, and growth",
      storyParagraphs: [
        "Nexa brings together women across different fields in a curated, accessible, and trustworthy directory. The goal is to make it easier for qualified professionals and potential clients to find one another with more clarity.",
        "For professionals, Nexa acts as a more organized public-facing presence: a place to present experience, specializations, and delivery style without depending entirely on the pace and format of social platforms.",
        "For clients, the platform reduces noise and makes research more practical. Instead of starting from scratch, they find structured profiles, clear categories, and more direct contact paths.",
      ],
      audienceEyebrow: "Who it serves",
      audienceTitle: "Nexa is built for both sides of the same connection",
      audienceItems: [
        "Women professionals, freelancers, and founders who want stronger visibility and better opportunities.",
        "People and companies looking for professionals with more trust, context, and ease.",
        "A network centered on thoughtful presentation, perceived quality, and stronger professional alignment.",
      ],
      ctaEyebrow: "Next step",
      ctaTitle: "Browse the platform or submit your application",
      ctaBody:
        "If you want to explore professionals already on Nexa, browse the directory. If you want your work represented on the platform, submit an application for review.",
      primaryCta: "Browse professionals",
      secondaryCta: "Apply to Nexa",
      robots: { index: false, follow: false },
    },
    footer: {
      about: "About Nexa",
      howItWorks: "How it works",
      categories: "Browse categories",
      professionals: "Browse professionals",
      applyNow: "Apply now",
      guidelines: "Guidelines",
      privacy: "Privacy",
      terms: "Terms of use",
    },
    cardCategoryCta: "Browse category",
    cardProfileCta: "View profile",
    directoryEmpty: "No professionals match this filter yet.",
    featuredEmpty: "No featured professionals match this filter yet.",
    verified: "Verified",
    remote: "Remote",
    local: "Local",
    remoteAndLocal: "Remote and local",
    all: "All",
    availability: "Availability",
    profileFallback: "This profile is currently available in Portuguese.",
    profileNotFoundTitle: "Profile not found",
    profileNotFoundText: "We couldn't find this professional.",
    backToProfessionals: "Back to professionals",
    browseAllProfessionals: "Browse all professionals",
    backToHome: "Back to home",
    sections: {
      about: "About",
      services: "Services",
      experience: "Experience",
      portfolio: "Portfolio",
      contact: "Contact",
    },
    labels: {
      location: "Location",
      languages: "Languages",
      profileType: "Profile type",
      delivery: "Delivery",
      engagement: "Engagement",
      experience: "Experience",
      clientFocus: "Client focus",
      projectsDelivered: "Projects delivered",
      email: "Email",
      website: "Website",
      social: "Social",
      safetyNote: "Safety note",
      profileDetail: "Profile detail",
      category: "Category",
    },
    contactViaNexa: "Contact via Nexa",
    contactModal: {
      title: "Contact via Nexa",
      close: "Close",
      name: "Name",
      namePlaceholder: "Your name",
      email: "Email",
      emailPlaceholder: "you@example.com",
      phone: "Phone",
      phonePlaceholder: "(000) 000-0000",
      message: "Message",
      messagePlaceholder: "Write your message",
      send: "Send",
      validation: "Please fill in Name and Email before sending your message.",
      success: "Message sent successfully",
      error: "We couldn't send your message right now. Please try again.",
    },
    applicationForm: {
      validation: "Please fill in the required fields before sending your application.",
      success:
        "Application submitted successfully! Your profile will be reviewed before it is published.",
      error: "We couldn't send your application right now. Please try again.",
      sending: "Sending...",
    },
    legal: {
      guidelines: {
        title: "Nexa Guidelines | Nexa",
        description: "Platform guidelines for professionals participating in Nexa.",
        heading: "Nexa Guidelines",
        intro: "Platform guidelines for professionals participating in Nexa.",
        robots: { index: false, follow: false },
        sections: [
          {
            title: "Nexa Platform Guidelines",
            paragraphs: [
              "Nexa was created to be a structured, safe, and trustworthy professional environment focused on increasing the visibility of women offering services.",
              "To preserve the quality and purpose of the platform, all professionals are expected to follow the guidelines below:",
            ],
          },
          {
            title: "1. Professionalism",
            paragraphs: [
              "Profiles should reflect clear professional practice, with objective descriptions of the services offered.",
              "The following is not allowed:",
            ],
            items: [
              "Offensive, discriminatory, or inappropriate content",
              "False or misleading information",
              "Excessively informal language or language incompatible with a professional environment",
            ],
          },
          {
            title: "2. Service clarity",
            paragraphs: ["Each professional should:"],
            items: [
              "Clearly describe services",
              "Indicate delivery mode (remote/local)",
              "Whenever possible, explain scope and engagement type",
            ],
          },
          {
            title: "3. Appropriate platform use",
            paragraphs: [
              "Nexa is a professional directory, not a social network.",
              "The following is not allowed:",
            ],
            items: [
              "Using the platform for purposes unrelated to service delivery",
              "Publishing personal content unrelated to professional work",
              "Using the platform for spam or self-promotion outside the scope of the profile",
            ],
          },
          {
            title: "4. Keeping information current",
            paragraphs: ["Each professional is responsible for:"],
            items: ["Keep information up to date", "Ensure presented data is accurate"],
          },
          {
            title: "5. Conduct",
            paragraphs: ["We expect all professionals to:"],
            items: [
              "Maintaining an ethical posture",
              "Respecting clients and other professionals",
              "Using the platform responsibly",
            ],
          },
        ],
      },
      privacy: {
        title: "Privacy Policy | Nexa",
        description: "Nexa privacy policy.",
        heading: "Privacy Policy",
        intro: "Nexa privacy policy.",
        robots: { index: false, follow: false },
        sections: [
          {
            title: "Nexa Privacy Policy",
            paragraphs: ["Nexa values the privacy and security of its users’ information."],
          },
          {
            title: "1. Data collection",
            paragraphs: ["Nexa only collects the information necessary for:"],
            items: [
              "Creating and displaying the professional profile",
              "Platform operation",
              "Communication with the user",
            ],
          },
          {
            title: "2. Use of information",
            paragraphs: ["The information provided may be used to:"],
            items: [
              "Displaying the professional profile on the platform",
              "Allowing clients to get in touch",
              "Improving the platform experience",
            ],
          },
          {
            title: "3. Data sharing",
            paragraphs: [
              "Nexa does not sell, rent, or share personal data with third parties for commercial purposes.",
              "Data may only be used:",
            ],
            items: ["For the platform’s internal operation", "When required by law"],
          },
          {
            title: "4. Public vs private data",
            items: [
              "Information included in the professional profile is public",
              "Sensitive personal information is neither requested nor displayed",
            ],
          },
          {
            title: "5. Security",
            paragraphs: [
              "Nexa adopts reasonable measures to protect user data, but cannot guarantee absolute security against unauthorized access.",
            ],
          },
          {
            title: "6. Data control",
            paragraphs: ["The user may:"],
            items: [
              "Request updates or removal of personal data",
              "End participation on the platform at any time",
            ],
          },
        ],
      },
      terms: {
        title: "Terms of Use | Nexa",
        description: "Nexa platform terms of use.",
        heading: "Terms of Use",
        intro: "Nexa platform terms of use.",
        robots: { index: false, follow: false },
        sections: [
          {
            title: "Nexa Terms of Use",
            paragraphs: [
              "Nexa reserves the right to review, approve, edit, or remove profiles at any time in order to preserve the quality and safety of the platform.",
            ],
          },
          {
            title: "1. Profile approval",
            items: [
              "All profiles are reviewed before publication",
              "Nexa may reject profiles that are not aligned with its guidelines",
            ],
          },
          {
            title: "2. Removal or suspension",
            paragraphs: ["Nexa may remove or suspend profiles in cases such as:"],
            items: [
              "Violation of the guidelines",
              "False or inconsistent information",
              "Improper use of the platform",
              "Inappropriate conduct",
            ],
          },
          {
            title: "3. Limitation of liability",
            paragraphs: ["Nexa acts as an intermediary for visibility and connection."],
            items: [
              "It does not participate in negotiations between professionals and clients",
              "It does not guarantee results, hires, or payments",
              "It is not responsible for agreements made outside the platform",
            ],
          },
          {
            title: "4. Contact between users",
            items: [
              "Contact between clients and professionals is the responsibility of the parties involved",
              "Nexa does not mediate conflicts",
            ],
          },
        ],
      },
    },
    robots: { index: false, follow: false },
  },
};

const LANGUAGE_NAME_MAP = {
  Arabic: { pt: "Árabe", en: "Arabic" },
  English: { pt: "Inglês", en: "English" },
  French: { pt: "Francês", en: "French" },
  Hindi: { pt: "Hindi", en: "Hindi" },
  Italian: { pt: "Italiano", en: "Italian" },
  Mandarin: { pt: "Mandarim", en: "Mandarin" },
  Portuguese: { pt: "Português", en: "Portuguese" },
  Spanish: { pt: "Espanhol", en: "Spanish" },
  Turkish: { pt: "Turco", en: "Turkish" },
  Urdu: { pt: "Urdu", en: "Urdu" },
};

export function getLangConfig(lang) {
  return UI[lang] || UI.pt;
}

export function getCategoryMeta(slug, lang) {
  return CATEGORY_META[slug]?.[lang] || CATEGORY_META[slug]?.pt;
}

export function getCurrentLang(lang) {
  return lang === "en" ? "en" : "pt";
}

export function normalizeBoolean(value) {
  return String(value).toLowerCase() === "true";
}

export function hasContent(value) {
  return String(value ?? "").trim() !== "";
}

export function getLocationMode(profile) {
  const value = (profile.remote_or_local || "").trim().toLowerCase();
  if (value === "remote") return "remote";
  if (value === "remote and local") return "hybrid";
  return "local";
}

export function matchesAvailabilityFilter(profile, filter) {
  const mode = getLocationMode(profile);
  if (filter === "all") return true;
  if (mode === "hybrid") {
    return filter === "remote" || filter === "local";
  }
  return mode === filter;
}

export function getLocalizedField(profile, baseKey, lang, fallback = "") {
  const normalizedLang = getCurrentLang(lang);
  const requested = profile[`${baseKey}_${normalizedLang}`];
  if (hasContent(requested)) {
    return requested;
  }

  if (normalizedLang === "en") {
    const portuguese = profile[`${baseKey}_pt`];
    if (hasContent(portuguese)) {
      return fallback || getLangConfig("en").profileFallback;
    }
  }

  return profile[`${baseKey}_pt`] || fallback;
}

export function getCategoryTitle(profile, lang) {
  return getLocalizedField(
    profile,
    "category",
    lang,
    getCategoryMeta(profile.category_slug, lang)?.title || "",
  );
}

export function getLocalizedLanguages(value, lang) {
  return String(value || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean)
    .map((item) => LANGUAGE_NAME_MAP[item]?.[lang] || item)
    .join(", ");
}

export function localizeDelivery(value, lang) {
  let output = String(value || "");
  if (lang === "pt") {
    if (output === "Remote and Local") return "Remoto e local";
    if (output === "Remote") return "Remoto";
    if (output === "Local") return "Local";
    output = output.replaceAll("On-site", "Presencial");
    output = output.replaceAll("Remote", "Remoto");
    output = output.replaceAll("Hybrid", "Híbrido");
  }
  return output;
}

export function localizeEngagement(value, lang) {
  let output = String(value || "");
  if (lang === "pt") {
    output = output.replaceAll("Project-based", "Por projeto");
    output = output.replaceAll("Retainer", "Recorrente");
  }
  return output;
}

export function getProfilePath(slug, lang) {
  return `/${getCurrentLang(lang)}/profile/${encodeURIComponent(slug)}`;
}

export function getLegacyProfileUrl(slug, lang) {
  return `/${getCurrentLang(lang)}/profile-template.html?slug=${encodeURIComponent(slug)}`;
}

export function getCategoryPath(slug, lang) {
  return `/${getCurrentLang(lang)}/category/${slug}`;
}

export function getLegacyCategoryUrl(slug, lang) {
  return `/${getCurrentLang(lang)}/category-${slug}.html`;
}

export function getLinkLabel(href, label, fallback) {
  if (hasContent(label)) return label;
  if (hasContent(href)) return String(href).replace(/^https?:\/\//, "");
  return fallback;
}

export function normalizeExternalUrl(value) {
  const candidate = String(value || "").trim();
  if (!candidate) return "";
  if (/^(https?:)?\/\//i.test(candidate)) {
    return candidate.startsWith("//") ? `https:${candidate}` : candidate;
  }
  if (/^(mailto:|tel:)/i.test(candidate)) {
    return candidate;
  }
  return `https://${candidate}`;
}

export function getBadgeLabel(profile, lang) {
  const ui = getLangConfig(lang);
  const mode = getLocationMode(profile);
  if (mode === "remote") {
    return ui.remote;
  }
  if (mode === "hybrid") {
    return ui.remoteAndLocal;
  }

  return (profile.location || "").split(",")[0] || ui.local;
}

export function getServiceTitle(profile, index, lang) {
  return getLocalizedField(profile, `service_${index}_title`, lang, "");
}

export function getServiceDescription(profile, index, lang) {
  return getLocalizedField(profile, `service_${index}_description`, lang, getLangConfig(lang).profileFallback);
}

export function getServiceEngagement(profile, index, lang) {
  return localizeEngagement(profile[`service_${index}_engagement`], lang);
}

export function getServiceDelivery(profile, index, lang) {
  return localizeDelivery(profile[`service_${index}_delivery`], lang);
}

export function getPortfolioItem(profile, index, lang) {
  return {
    title: getLocalizedField(profile, `portfolio_${index}_title`, lang, ""),
    description: getLocalizedField(profile, `portfolio_${index}_description`, lang, getLangConfig(lang).profileFallback),
    url: String(profile[`portfolio_${index}_url`] || "").trim(),
  };
}

export function buildServiceTags(profile, lang) {
  return [1, 2, 3].map((index) => getServiceTitle(profile, index, lang)).filter(Boolean);
}

export function matchesDirectorySearch(profile, query, lang) {
  if (!query) return true;

  const terms = [
    profile.name,
    getCategoryTitle(profile, lang),
    getLocalizedField(profile, "role_title", lang, ""),
    getLocalizedField(profile, "short_bio", lang, ""),
    profile.location,
    getLocalizedLanguages(profile.languages, lang),
    ...buildServiceTags(profile, lang),
  ];

  return terms.join(" ").toLowerCase().includes(query);
}
