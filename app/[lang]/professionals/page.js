    ...
    <main>
      <section className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-nexa_orange">{ui.professionalsPage.eyebrow}</p>
        ...
        <div className="mt-10 flex flex-wrap gap-3 text-sm font-semibold">
          {CATEGORY_ORDER.map((slug) => (
            <a
              key={slug}
              href={`#${slug}`}
              className="rounded-full border border-charcoal/15 bg-white px-4 py-2 shadow-soft transition-colors hover:border-nexa_orange hover:text-nexa_orange"
            >
